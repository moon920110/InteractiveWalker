...
parser.add_argument('--exp_dir', type=str, default='../data/train_200/', help='Experiment path')
parser.add_argument('--exp', type=str, default='all_exercises', help='Name of experiment')
...
parser.add_argument('--epoch', type=int, default=1000, help='The time steps you want to subsample the dataset to,500')
parser.add_argument('--ckpt', type=str, default='all_exercises_0.0001_10_best', help='loaded ckpt file')
parser.add_argument('--eval', type=bool, default=False, help='Set true if eval time')
parser.add_argument('--test_dir', type=str, default='../data/test_200/', help='test data path')
parser.add_argument('--val_dir', type=str, default='../data/val_200/', help='val data path')
...
if args.linkLoss:
    link_min = pickle.load(open('./link_min_all.p', "rb"))
    link_max = pickle.load(open('./link_max_all.p', "rb"))
...
if not args.eval:
    ...
    mask = []
    train_dataset = sample_data(train_path, args.window, mask, args.subsample)  # tactile, heatmap, keypoint, tactile_frame, idx
...
if __name__ == '__main__':
    np.random.seed(0)
    torch.manual_seed(0)
    # model = tile2openpose_conv3d(args.window)
    model = nn.DataParallel(tile2openpose_conv3d(args.window)) # model
    ...
    if args.eval:
        ...
        for i_batch, sample_batched in bar(enumerate(test_dataloader, 0)):
            ...
            if args.exp_data:
                ...
                if i_batch % 20 and i_batch != 0: #set the limit to avoid overflow
                    c += 1
        ...
        if args.exp_video:
            ...
            generateVideo(to_save,
                  args.exp_dir + 'predictions/video/' + args.ckpt + '_1',
                  heatmap=True)
    ...
    if args.train_continue:
        ...
        # best_val_loss = 0.0254
        best_val_loss = 0.0707
    ...
    for epoch in range(args.epoch): #500
        ...
        for i_batch, sample_batched in bar(enumerate(train_dataloader, 0)):
            ...
            tactile = sample_batched[0].clone().detach().to(dtype=torch.float, device=device)
            heatmap = sample_batched[1].clone().detach().to(dtype=torch.float, device=device)
            keypoint = sample_batched[2].clone().detach().to(dtype=torch.float, device=device)
            idx = sample_batched[4].clone().detach().to(dtype=torch.float, device=device)
            ...
            if i_batch % int(len(train_dataloader)//3) == 0 and i_batch != 0:
                ...
                keypoint_l2 = []
                print("val_len", len(val_dataloader))
