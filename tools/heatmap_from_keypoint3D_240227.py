...
from preprocessing_utils import softmax, normalize_with_range
...
def plot3Dheatmap(data, seperate):
    ...
    #colors = ['Reds', 'Blues', 'Blues', 'Blues', 'Blues',
    ...
    if seperate:
        ...
        for i in range(21):
            ...
            frame = np.reshape(data[i,:,:,:],(data.shape[1],data.shape[2],data.shape[3]))  #data는 gaussian 돌린값 저장중
            flag = frame > 0
...
def heatmap_from_keypoint(keypoint_path, xyz_range, heatmap_size):
    ...
    b = np.array([[x_range[0], y_range[0], z_range[0]]])                        #range min값
    threshold = [0,1,0,1,0,1]
    keypoint = normalize_with_range((keypoint - b)/resolution, max(size)-1, 0)  #0~1로 normalize 함, data = (data-min)/(max-min)
    keypoint = remove_keypoint_artifact(keypoint, threshold)                    #바깥 keypoint를 range안으로 집어넣기

    # print (np.amax(keypoint), np.amin(keypoint))                                #min max값 출력, 바깥 keypoint확인하려는듯?
    ...
    for i in range(keypoint.shape[0]):
        ...
        for k in range(21):
            dis = np.sqrt((pos_x-frame[k,0])**2 + (pos_y-frame[k,1])**2 + (pos_z-frame[k,2])**2)  #pos_x는 격자 x좌표array, dis는 각 격자에서의 거리 값인듯?
            g = gaussian(dis, 0.001, 1)
            # heatmap[i,k,:,:,:] = round_to_1(softmax(g),2)
            heatmap[i,k,:,:,:] = softmax(g) /0.25 #1:0.25; 0.5:0.8         #heatmap[frame,관절(21), x(20),y(20),z(18)(각 격자에서의 거리값에 가우시안 처리한거)]
...
def heatmap_gen(path:str) -> None:
    path_list = [path]

# path_list = ['./data/2022-11-09_20-09-21_testing_carpets_JH_squat_2/']
# path_list = ['./']
    ...
    for num, p in enumerate(path_list):
        ...
        # xyz_range = [[-100, 100], [-100, 400], [-200, 0]]
        xyz_range = [[-1200, -200], [2000, 3000], [600, 1500]]
        size = [20, 20, 18] #define 3D space
        keypoint_path_list = [path + '/fitted_keypoints.p']
        ...
        out = cv2.VideoWriter(path + '/heatmap_full.avi', fourcc, 10, (1280,480))
